#!/bin/bash
# Archive cleanup — 수동 트리거
# 사용법: bash scripts/archive-cleanup.sh
  ...
  if [ "$status" = "done" ]; then
    ...
    if [ -n "$updated" ]; then
      ...
      if [ "$days_ago" -ge 30 ]; then
        ...
        echo "| $TODAY | Tickets/$name | Archive/tickets/$MONTH/$name | done 30일+ 초과 |" >> "$VAULT/Archive/archive-log.md"
...
for f in "$VAULT/Daily/"*.md; do
  ...
  if [ -n "$name" ]; then
    ...
    if [ "$days_ago" -ge 30 ]; then
      ...
      echo "| $TODAY | Daily/$(basename $f) | Archive/daily/$YEAR/$(basename $f) | 30일+ 초과 |" >> "$VAULT/Archive/archive-log.md"
...
for f in "$VAULT/Sessions/"*.md; do
  ...
  if [ -n "$date_part" ]; then
    ...
    if [ "$days_ago" -ge 60 ]; then
      ...
      echo "| $TODAY | Sessions/$(basename $f) | Archive/sessions/$YEAR/$(basename $f) | 60일+ 초과 |" >> "$VAULT/Archive/archive-log.md"
      echo "archived: $(basename $f) ($days_ago days)"
      CHANGED=1
    fi
  fi
...
if [ "$CHANGED" -eq 0 ]; then
  ...
  # INDEX.md 재생성
  bash ~/.openclaw/workspace/scripts/regen-ticket-index.sh 2>/dev/null
  cd "$VAULT" && git add -A && git commit -m "archive: cleanup $TODAY" && git push
  echo "아카이브 완료 + git push"
fi
